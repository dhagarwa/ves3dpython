function [] = ellipsoidSLBendingTest5()
      clc
    ...
    fid = fopen(fullfile(rootdir, relativefolder, 'ellipsoid3_matlabtrg32.txt'), 'rt');
    C = textscan(fid,'%f');
    fclose(fid);
    %celldisp(C);
    A = cell2mat(C);
    ...
    rootdir = '/Users/dhwanit/Google Drive/Biros Research/ves3dpython/matlab';
    relativefolder = 'testfiles';
    fid = fopen(fullfile(rootdir, relativefolder, 'test_ellipsoid3_SL_fb_matlabtrg32_p64.txt'), 'rt');
    C = textscan(fid,'%f');
    fclose(fid);
    %celldisp(C);
    A = cell2mat(C);
...
%     error_src_trg = max(vecnorm(all_pot - all_pot_trg,2,2))/max(vecnorm(all_pot,2,2))
    ...
    for i=1:6
       patch =  standardEllipsoidPatch(m, n, i, R, 1, 0.9, 1);
...
%     all_r = all_r';
%     all_r = all_r(:);
%     dlmwrite('ellipsoid3_matlabtrg32.txt',all_r,'newline','pc', 'precision', 12);
%     figure;
%     for i=1:6
%         p = patches(i);
