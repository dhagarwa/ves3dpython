function p = standardEllipsoidPatchSymmetrical(m, n,  numPatch, R, a, b, c)
...
p = Patch(m, n, R, numPatch, []);
    ...
    if numPatch==1 
        p.r = [a*cos(p.u), b*sin(p.u).*sin(p.v), -c*sin(p.u).*cos(p.v)];
        p.r_ = [a*cos(p.u_), b*sin(p.u_).*sin(p.v_), -c*sin(p.u_).*cos(p.v_)];
        p.rb = [a*cos(p.U), b*sin(p.U).*sin(p.V), -c*sin(p.U).*cos(p.V)];
    elseif numPatch==2
