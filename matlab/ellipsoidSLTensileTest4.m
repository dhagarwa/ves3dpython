function [] = ellipsoidSLTensileTest4()
      clc
    ...
    fid = fopen(fullfile(rootdir, relativefolder, 'ellipsoid2_src64.txt'), 'rt');
    C = textscan(fid,'%f');
    fclose(fid);
    %celldisp(C);
    A = cell2mat(C);
    ...
    rootdir = '/Users/dhwanit/Google Drive/Biros Research/ves3dpython/matlab';
    relativefolder = 'testfiles';
    fid = fopen(fullfile(rootdir, relativefolder, 'test_ellipsoid2_SL_fs_src64.txt'), 'rt');
    C = textscan(fid,'%f');
    fclose(fid);
    %celldisp(C);
    A = cell2mat(C);
