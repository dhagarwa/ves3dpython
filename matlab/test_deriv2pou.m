function [] = test_deriv2pou()
%test resolution of double derivative of pou function 
%see how any frequncies resolve it accurately
    ...
    [pou1_du, pou1_dv, pou1_du2] = doubderiv_pou(p1, p1.numPatch, p1.r);
%     figure;
%     stem3(reshape(p1.pou,[p1.Nu,p1.Nv]));
     figure;
     stem3(reshape(pou1_du2,[p1.Nu,p1.Nv]));
%     figure;
%     stem3(reshape(pou1_dv,[p1.Nu,p1.Nv]));
    
    [pou1_du_app, pou1_dv_app] = p1.grad_patch(p1.pou);
    [pou1_du2_app, pou1_duv_app] = p1.grad_patch(pou1_du_app);
    
     figure;
     stem3(reshape(pou1_du2_app,[p1.Nu,p1.Nv]));
%     figure;
...
%     
    ...
    err_du2 = max(abs(pou1_du2_app-pou1_du2))
...
function [du, dv, du2] = doubderiv_pou(obj, patch, r) %patch is patch num here
    [v1, u1, rho1] = cart2sph(r(:,1), r(:,2), r(:,3));
    %u = pi/2 - u;
    %u; v;
    if patch==1
    ...
    for ii=1:nn
        ...
        elseif t ==0
            ...
            du(ii) = dphi_dt*dt_du;
            dv(ii) = dphi_dt*dt_dv;
            d2phi_dt2 = phi*(dt^2 + 2*exp(-1/t)/(t^4*(t-1)) - 4*exp(-1/t)/(t^3*(t-1)) - 4*exp(-1/t)/(t^2*(t-1)^2) + 4*exp(-1/t)/(t-1)^3);
            d2t_du2 = (1/d)*(sin(u)*sin(v)/(1-sin(u)^2*sin(v)^2)^(0.5) - sin(u)*cos(u)^2*sin(v)^3/(1-sin(u)^2*sin(v)^2)^(3/2));
            du2(ii) = d2phi_dt2*(dt_du)^2 + dphi_dt*d2t_du2;
